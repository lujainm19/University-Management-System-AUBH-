...
public class TeacherLeave extends JFrame implements ActionListener{
    ...
    TeacherLeave(){
        ...
        JLabel lblIDno = new JLabel("Search by Faculty ID");
        lblIDno.setBounds(60, 100, 200, 20);
        lblIDno.setFont(new Font("Tahoma", Font.PLAIN, 18));
        add(lblIDno);

        cempIDno = new Choice();
        cempIDno.setBounds(60, 130, 200, 20);
        add(cempIDno);
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == submit) {
            ...
            String empIDno = cempIDno.getSelectedItem();
            String date = ((JTextField) dcdate.getDateEditor().getUiComponent()).getText();
            String duration = ctime.getSelectedItem();

            //query made to store values in studentleave table
            String query = "insert into facultyleave values('"+empIDno+"', '"+date+"', '"+duration+"')";
